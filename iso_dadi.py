...
## downsample to 80% of minimum
downs = ns * 0.8
dns = numpy.array([int(downs[0]), int(downs[1])])
fs = fs.project(dns)

of = "dd_80_"+p1+"_"+p2+"_iso.txt"
pngf = "dd_80_"+p1+"_"+p2+"_iso.png"

ofile = open(of,"w")

ns =  fs.sample_sizes

## define split model, similar to dadi website, includes exponential growth
...
# Parameters are: (s, nu1, nu2, T)
upper_bound = [.9, 10, 10, 10]
